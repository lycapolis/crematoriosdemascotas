<?php
/**
 * AJAX — Anclar/desanclar portada principal de un crematorio
...
if ($imagenId > 0) {
    $check = $pdo->prepare("SELECT id FROM crematorio_imagenes WHERE id = :img AND crematorio_id = :crem");
...
$pdo->prepare("UPDATE crematorios SET portada_principal_id = :val WHERE id = :id")
    ->execute([':val' => $nuevoValor, ':id' => $crematorioId]);

echo json_encode(['ok' => true, 'portada_principal_id' => $nuevoValor]);
